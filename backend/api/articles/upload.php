...
if (!in_array($mimeType, $allowedTypes)) {
    jsonResponse(400, '不支持的文件类型: ' . $mimeType . '，仅支持 JPG, PNG, GIF, WebP');
}

// 验证文件大小 (最大 10MB)
$maxSize = 10 * 1024 * 1024;
if ($file['size'] > $maxSize) {
    jsonResponse(400, '文件大小超过 10MB 限制');
}
...
$filename = 'blog_' . time() . '_' . $random . '.' . strtolower($ext);
$filepath = $uploadDir . $filename;

// 移动上传文件
if (!move_uploaded_file($file['tmp_name'], $filepath)) {
    jsonResponse(500, '保存文件失败，请检查 uploads 目录权限');
}
