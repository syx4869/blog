...
设计 MySQL 数据模型与 RESTful API 接口规范，实现前后端分离内容管理系统。前端采用 Vue 3 + Vite 构建，后端使用原生 PHP + MySQL 提供 RESTful API 接口。项目特色包括每日随机壁纸背景、液态玻璃导航栏效果、富文本/HTML 双模式文章编辑器、IP 防重复的点赞/收藏系统等。
...
┌─────────────────────────────────────────┐
│           API 层 (RESTful)               │
│  /api/articles  /api/comments           │
│  /api/wallpapers /api/auth              │
│  /api/home/stats /api/about             │
└──────────────────┬──────────────────────┘
                   │ PDO + 预处理语句
                   │ 事务处理 / 防SQL注入
                   ▼
...
│          数据层 (MySQL)                  │
│  ┌─────────┐ ┌─────────┐ ┌───────────┐ │
│  │articles │ │ comments│ │  admins   │ │
│  └─────────┘ └─────────┘ └───────────┘ │
│  ┌─────────┐ ┌─────────┐ ┌───────────┐ │
│  │  tags   │ │wallpaper│ │   likes   │ │
│  └─────────┘ └─────────┘ └───────────┘ │
└─────────────────────────────────────────┘
```
...
| 亮点              | 实现方式                                                              |
| --------------- | ----------------------------------------------------------------- |
| **统一请求封装**      | 基于 `fetch` + `AbortController` 封装 request 请求模块，实现统一错误处理与 15s 超时控制 |
| **多对多标签关联**     | 设计 `article_tags` 中间表实现文章与标签的多对多关系，支持动态创建与关联查询                    |
| **嵌套评论结构**      | 基于 `parent_id` 自关联实现嵌套评论（二级回复），支持点赞与作者标识                          |
| **壁纸缓存策略**      | 使用 `localStorage` 缓存每日壁纸数据，按日期键存储，减少重复请求                          |
| **IP 防重复机制**    | 基于 `IP + 唯一索引` 实现点赞/收藏防重复，同一 IP 只能操作一次，再次操作自动取消                   |
| **液态玻璃效果**      | SVG `<filter>` + Canvas 位移贴图实现导航栏液态玻璃质感，支持动态模糊                    |
| **双模式编辑器**      | 管理后台集成 Quill 富文本编辑器与 HTML 源码编辑，双向实时同步                             |
| **JWT-less 认证** | 基于数据库 Token + Cookie/Session 双通道实现后台登录，支持记住我与自动续期                 |
...
| cover\_image   | VARCHAR(500) | 封面图 URL  |
| category\_id   | INT          | 分类 ID    |
| icon           | VARCHAR(50)  | 图标       |
| view\_count    | INT          | 浏览量      |
| like\_count    | INT          | 点赞数      |
| collect\_count | INT          | 收藏数      |
| is\_published  | TINYINT      | 是否发布     |
...
| ----------- | -------------------------- | -------------- |
| id          | INT PK                     | 主键             |
| article\_id | INT                        | 文章 ID          |
| ip\_address | VARCHAR(45)                | IP 地址          |
| user\_agent | VARCHAR(500)               | UA             |
